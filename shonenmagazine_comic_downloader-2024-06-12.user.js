...
(function() {
    ...
    let autoCapturedSeed = null;

    JSON.parse = function(text, reviver) {
        const obj = originalParse(text, reviver);
        if (obj && typeof obj === 'object') {
            if (obj.scramble_seed) {
                if (autoCapturedSeed !== obj.scramble_seed) {
                    autoCapturedSeed = obj.scramble_seed;
                    if (window.autoFillSeed) window.autoFillSeed(obj.scramble_seed);
                }
            }
            for (let key in obj) {
                if (obj[key] && obj[key].scramble_seed) {
                    if (autoCapturedSeed !== obj[key].scramble_seed) {
                        autoCapturedSeed = obj[key].scramble_seed;
                        if (window.autoFillSeed) window.autoFillSeed(obj[key].scramble_seed);
                    }
                }
            }
        }
    ...
    const createUI = () => {
        ...
        window.autoFillSeed = (seed) => {
            CONFIG.RESTORE.scrambleSeed = seed;
            if (viewerUI) {
                const seedInput = viewerUI.querySelector(".seed-input");
        ...
        if (autoCapturedSeed) {
            window.autoFillSeed(autoCapturedSeed);
        }
    };
